...
class NLPWrapper:
    ...
    def extractFeaturesFromText(self):
        ...
        query = db.query(f'SELECT * FROM mindsdb.count_swears_2 WHERE script = \'{self.transcribed_text}\'')
        swear_count = query.fetch()
        query = db.query(f'SELECT * FROM mindsdb.sentiment_topics WHERE script = \'{self.transcribed_text}\'')
        topics = query.fetch()
        query = db.query(f'SELECT * FROM mindsdb.languages WHERE script = \'{self.transcribed_text}\'')
        langs = query.fetch()
        query = db.query(f'SELECT * FROM mindsdb.brand_safety WHERE script = \'{self.transcribed_text}\'')
        score = query.fetch()
